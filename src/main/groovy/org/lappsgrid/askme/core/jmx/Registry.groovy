...
class Registry {
    ...
    static Histogram histogram(String... name) {
        return registry.histogram(Registry.name(name))
    }

    static <T> Gauge<T> gauge(String name, Closure closure) {
        Gauge<T> gauge = new Gauge() {
            @Override
            T getValue() {
                return closure()
            }
